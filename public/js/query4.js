...
app.controller('AppCtrl', function AppCtrl($scope, $location, $http) {
  ...
  vm.queries = [];
  vm.arrow=true;
  $http.get('http://192.168.0.12:3000/queries/4')
  .then(function(response) {
        vm.data = response.data;
        for (var i = 0; i < vm.data.length; i++) {
          vm.queries.push({index: i+1, name: vm.data[i].name, count: vm.data[i].count});
          vm.queriesArray(1);
        }
    });
  ...
  vm.previous = function(){
    location.assign('query3.html');
  }

  vm.next = function(){
    location.assign('query5.html');
  }
});

...
app.controller('AppCtrl', function AppCtrl($scope, $location, $http) {
  ...
  vm.queries = [];
  vm.arrow=true;
  $http.get('http://192.168.0.12:3000/queries/5')
  .then(function(response) {
        vm.data = response.data;
        for (var i = 0; i < vm.data.length; i++) {
          vm.queries.push({index: i+1, airline: vm.data[i].airline, count: vm.data[i].count});
          vm.queriesArray(1);
        }
    });
  ...
  vm.previous = function(){
    location.assign('query4.html');
  }

  vm.next = function(){
    location.assign('query6.html');
  }
});
